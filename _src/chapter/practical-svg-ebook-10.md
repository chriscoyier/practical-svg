...
The “without cutting off” part is another aspect of `preserveAspectRatio`. The default value is `xMidYMid` `meet`—note the “meet.” You can replace `meet` with `slice` to say instead: _Fill the area entirely; cutting off is okay_.
